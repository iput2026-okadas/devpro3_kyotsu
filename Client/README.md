...
この JSON の末尾に改行を付け、5秒間隔で完全なデータを6回送信します。いずれかの
センサー値が取得できなかった測定回は送信せず、次の5秒周期で再度測定します。
...
- DHT22 の読み取りに失敗した場合、`client.py` 内では `temp` と `humid` を `None`
  として扱います。
- DHT22 は送信ごとに1回だけ読み取り、失敗時の再試行や前回値の再利用は行いません。
  診断用に `python dht22.py` で読み取り状況を確認できます。
- CO2 や光量も1回だけ読み取り、失敗時は該当値を `None` として扱います。
- 4項目のいずれかが `None` の測定回はサーバーへ送信しません。0による補完や前回値の
  再利用は行わず、次の測定機会に取得できた完全なデータだけを送信します。
