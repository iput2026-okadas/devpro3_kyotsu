...
def make_rows(client_id, count, interval, scenario):
...
def write_csv(path, client_id, count, interval, scenario):
    rows = make_rows(client_id, count, interval, scenario)
...
def send_json(host, port, client_id, count, interval, scenario):
...
def print_json(client_id, count, scenario):
    for index in range(count):
        data = make_sensor_data(index, count, scenario)
        data["client_id"] = client_id
...
def interactive_args():
    ...
    if output_mode == "send":
        ...
        client_id = prompt_text("クライアントID", client_id)
...
def parse_args():
    ...
    parser.add_argument("--client-id", default="fake-sensor")
...
def main():
    ...
    if args.csv_out:
        ...
        print_json(args.client_id, args.count, args.scenario)
    else:
        send_json(
            args.host,
            args.port,
