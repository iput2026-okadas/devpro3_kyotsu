...
4つのセンサー値のいずれかが欠落または `null` のデータはCSVへ記録せず、通知処理にも
渡しません。次に受信した完全なデータだけを処理します。
...
平均温度・平均湿度・平均CO2・平均光量の表示、CSVまたはJSON形式でのエクスポート、
センサーデータの追加ができます。
新しいCSVでは `client_id` 列から各行の送信元を確認できます。従来の `client_id` 列が
ないCSVも引き続き表示できます。

「データ追加」では、選択中のCSVに存在する `client_id` を選び、秒単位の
`timestamp`、`temp`、`humid`、`co2`、`light_percent` を入力します。従来形式の
CSVでは `id` 列をクライアントIDとして扱います。`co2` または `light_percent` 列が
