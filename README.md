...
書き出します。受信データは通知判定モジュールにも渡され、CO2 濃度に応じて
Discord または Slack へ Webhook 通知できます。サーバー側には、CSV 表示用の
Flask アプリと、気温変化の原因を分類する機械学習デモも含まれています。
...
CO2 濃度に応じた Discord または Slack 通知を使う場合は、実行環境で
`WEBHOOK_URL` を設定してから `Sever/server.py` を起動します。実際の Webhook URL は
コミットしないでください。`Sever/server.py` が TCP で受信したデータを
`Sever/alert.py` の通知判定関数へ渡します。
