...
- `Client/client.py` が送信する JSON キーは `temp`, `humid`, `co2`,
  `light_percent` です。
- クライアントは JSON の末尾に改行を付け、完全なデータを5秒間隔で6回送信します。
  いずれかのセンサー値が取得できなかった測定回は送信せず、次の機会に再度測定します。
- `Server/server.py` が CSV に保存する列は `id`, `timestamp`, `temp`, `humid`
  のみです。`co2` と `light_percent` は CSV には保存されません。
- 4つのセンサー値のいずれかが取得できない測定回はクライアントから送信せず、
  サーバー側でも不完全な受信データをCSVへ記録しません。0や前回値による補完は
  行わず、次の測定機会の完全なデータを使用します。
