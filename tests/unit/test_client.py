...
class TestClient(unittest.TestCase):
    def setUp(self):
        ...
        self.dht22 = types.SimpleNamespace(
            get_dht_data_with_status=MagicMock(return_value=(24.5, 60.0, "ok", None)),
            close=MagicMock(),
        )
    ...
    def test_read_sensors_keeps_other_values_when_dht22_fails(self):
        ...
        self.assertEqual(result["temp"], None)
        self.assertEqual(result["humid"], None)
    ...
    def test_read_sensors_keeps_other_values_when_co2_fails(self):
        ...
        self.assertEqual(result["co2"], None)
        self.assertEqual(result["light_percent"], 55.5)
    ...
    def test_read_sensors_keeps_other_values_when_light_fails(self):
        ...
        self.assertEqual(result["humid"], 60.0)
        self.assertEqual(result["co2"], 900)
        self.assertEqual(result["light_percent"], None)
    ...
    def test_client_test_sends_newline_delimited_json_six_times(self):
        ...
        with patch.object(self.client.socket, "socket", return_value=socket_context):
            with patch.object(self.client, "read_sensors", return_value=sensor_data):
                with patch.object(self.client.time, "sleep") as mock_sleep:
                    with patch("sys.stdout"):
                        self.client.client_test("127.0.0.1", 8765)

        socket_instance.setsockopt.assert_called_once_with(
            self.client.socket.SOL_SOCKET,
            self.client.socket.SO_REUSEADDR,
            1,
        )
