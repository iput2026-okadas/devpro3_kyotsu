...
class TestAlert(unittest.TestCase):
    def setUp(self):
        ...
        # 実際のWebhookへ送信せず、通知処理を通すためのダミーURLを設定する
        self.original_webhook_url = alert.SLACK_WEBHOOK_URL
        alert.SLACK_WEBHOOK_URL = "https://example.invalid/slack-webhook"
        self.addCleanup(
            setattr,
            alert,
            "SLACK_WEBHOOK_URL",
            self.original_webhook_url,
        )
    ...
    def test_send_alert_warning(self, mock_post):
        ...
        self.assertIn("注意", kwargs["json"]["text"])
    ...
    def test_send_alert_danger(self, mock_post):
        ...
        mock_post.assert_called_once()
        _, kwargs = mock_post.call_args
        self.assertIn("警告", kwargs["json"]["text"])
        mock_post.return_value.raise_for_status.assert_called_once_with()
    ...
    def test_process_sensor_data_danger_to_recover(self, mock_post):
        ...
        mock_post.assert_called_once()
        _, kwargs = mock_post.call_args
        self.assertIn("回復", kwargs["json"]["text"])
